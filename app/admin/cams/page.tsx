...
const emptyForm = { name: "", surname: "", email: "", cell: "" };
...
export default function ManageCAMs() {
  ...
  const startEdit = (cam: CAM) => {
    setEditId(cam.id);
    setEditForm({ name: cam.name, surname: cam.surname, email: cam.email, cell: cam.cell });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-oj-blue mb-1">Manage CAMs</h1>
        <p className="text-sm text-oj-muted">Client Account Managers available in the onboarding intake form.</p>
      </div>
      ...
      <div className="bg-oj-white border border-oj-border rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-sm text-oj-muted">No CAMs added yet.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-oj-border bg-oj-bg">
                {["Name", "Surname", "Email", "Cell", ""].map((h) => (
                  <th key={h} className="text-left px-4 py-3 text-xs font-bold text-oj-muted uppercase tracking-wider">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {cams.map((cam, i) =>
                editId === cam.id ? (
                  <tr key={cam.id} className={`border-b border-oj-border ${i % 2 === 1 ? "bg-oj-bg/50" : ""}`}>
                    <td className="px-4 py-2">
                      <input
                        value={editForm.name}
                        onChange={(e) => setEditForm((f) => ({ ...f, name: e.target.value }))}
                        className="w-full border border-oj-border rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-oj-blue"
                      />
                    </td>
                    <td className="px-4 py-2">
                      <input
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex gap-2 justify-end">
                        <button onClick={() => handleSaveEdit(cam.id)} disabled={saving} className="text-xs font-semibold text-oj-blue hover:text-oj-blue-hover">Save</button>
                        <button onClick={() => setEditId(null)} className="text-xs text-oj-muted hover:text-oj-dark">Cancel</button>
                      </div>
                    </td>
                  </tr>
                ) : (
                  <tr key={cam.id} className={`border-b border-oj-border last:border-0 ${i % 2 === 1 ? "bg-oj-bg/50" : ""}`}>
                    <td className="px-4 py-3 font-medium text-oj-dark">{cam.name}</td>
                    <td className="px-4 py-3 text-oj-dark">{cam.surname}</td>
                    <td className="px-4 py-3 text-oj-muted">{cam.email}</td>
                    <td className="px-4 py-3 text-oj-muted">{cam.cell || "—"}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex gap-3 justify-end">
                        <button onClick={() => startEdit(cam)} className="text-xs font-semibold text-oj-blue hover:text-oj-blue-hover">Edit</button>
                        <button onClick={() => handleDelete(cam.id)} className="text-xs font-semibold text-oj-orange hover:text-oj-orange-hover">Remove</button>
                      </div>
                    </td>
                  </tr>
                )
