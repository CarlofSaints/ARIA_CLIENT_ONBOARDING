...
// Channel multi-select dropdown — portal-based to escape table overflow clipping
function ChannelDropdown({ channels, selected, onChange }: {
  ...
  // Keep panel position in sync while open (scroll / resize)
  useEffect(() => {
    if (!open) return;
    function update() {
  ...
  const panelStyle: React.CSSProperties = rect
    ? { position: "fixed", top: rect.bottom + 4, left: rect.left, width: 256, zIndex: 9999 }
    : { display: "none" };
  ...
  return (
    <div>
      <button
        ...
        className="w-full min-w-[200px] text-left text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white hover:border-[#3D6273] focus:outline-none focus:ring-2 focus:ring-[#3D6273] focus:ring-offset-1 flex items-center justify-between gap-2"
      >
      ...
      {selectedNames.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1.5">
          {selectedNames.map((name) => (
            <span key={name} className="text-xs bg-[#3D6273] text-white px-2 py-0.5 rounded-full font-medium">
              {name}
            </span>
          ))}
        </div>
      ...
      {/* Dropdown panel rendered in a portal to escape table overflow clipping */}
            ...
            <div className="max-h-56 overflow-y-auto divide-y divide-gray-100">
              {channels.map((ch) => {
                ...
                return (
                  <label
                    key={ch.id}
                    className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${checked ? "bg-[#eef4f7]" : "hover:bg-gray-50"}`}
                  >
...
export default function PersonnelFormPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = use(params);

  const [status, setStatus] = useState<"loading" | "notFound" | "form" | "submitted">("loading");
  ...
  useEffect(() => {
    fetch(`/api/personnel/${token}`)
      ...
      .then((data: PageData | null) => {
        if (!data) return;
        setPageData(data);
        // Pre-populate rows from existing submission if available
        if (data.submission && data.submission.length > 0) {
          setRows(data.submission.map((r) => ({
            ...
            cell: r.cell ?? "",
            channels: r.channels ?? [],
            principal: r.principal ?? data.clientName,
            brand: r.brand ?? data.clientName,
            customFields: r.customFields ?? {},
          })));
        } else {
          setRows([makeRow(data.clientName)]);
        }
        setStatus(data.alreadySubmitted ? "submitted" : "form");
      })
  ...
  const validate = (): boolean => {
    ...
    rows.forEach((row) => {
      ...
      // custom fields
      pageData?.personnelConfig.customFields
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError("");
    try {
      const res = await fetch(`/api/personnel/${token}`, {
        method: "POST",
  ...
  if (status === "loading") {
    return (
      <div className="min-h-screen bg-[#F5F7F8] flex items-center justify-center">
        <div className="flex items-center gap-3 text-[#3D6273]">
    ...
    return (
      <div className="min-h-screen bg-[#F5F7F8] flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-10 max-w-sm w-full text-center">
          ...
          <p className="text-sm text-gray-500">This link is invalid or has expired. Please contact your Account Manager.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-[#F5F7F8] flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl border border-green-200 shadow-sm p-10 max-w-md w-full text-center">
          ...
          <p className="text-sm text-gray-600 leading-relaxed mb-2">
            Your information has been received and saved.
          </p>
          ...
          {pageData?.submittedAt && (
            <p className="text-xs text-gray-400 mt-4">
              Submitted: {new Date(pageData.submittedAt).toLocaleDateString("en-ZA", { dateStyle: "long" })}
            </p>
          )}
          <div className="mt-6 pt-6 border-t border-gray-100">
            <p className="text-xs text-gray-400 mb-3">Need to make a change?</p>
            <button
              onClick={() => setStatus("form")}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-[#3D6273] text-[#3D6273] text-sm font-semibold hover:bg-[#eef4f7] transition-colors"
            >
              ✏ Edit Submission
            </button>
          </div>
  ...
  // Form view
  const roleOptions = pageData!.personnelConfig.roleOptions
    .filter((r) => r.active)
    .sort((a, b) => a.order - b.order);
  const activeCustomFields = pageData!.personnelConfig.customFields
    .filter((f) => f.active)
    .sort((a, b) => a.order - b.order);
  ...
  return (
    <div className="min-h-screen bg-[#F5F7F8]">
      {/* Header */}
      <div className="bg-[#3D6273] text-white px-6 py-5">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <img src="/aria-logo.png" alt="ARIA" className="h-8 object-contain" />
            <img src="/oj-logo.jpg" alt="OuterJoin" className="h-8 object-contain rounded" />
          </div>
          <h1 className="text-2xl font-bold">Personnel Information</h1>
          <p className="text-sm text-[#d0e6ef] mt-1">{pageData!.clientName}</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-gray-100">
            <p className="text-sm text-gray-600 leading-relaxed">
              Please fill in the details of all key personnel for your account. Add one row per person.
              All fields marked with <span className="text-red-500">*</span> are required.
            </p>
          </div>

          {/* Already submitted warning */}
          {pageData!.alreadySubmitted && (
            <div className="mx-6 mt-4 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
              <strong>Note:</strong> This form has already been submitted. Submitting again will replace the previous response.
            </div>
          )}
                  ...
                  <tr key={row._key} className="hover:bg-[#F5F7F8]/50">
                    ...
                    <td className="px-4 py-3">
                      <input
                        type="tel"
                        value={row.cell}
                        onChange={(e) => updateRow(row._key, "cell", e.target.value)}
                    ...
                    <td className="px-4 py-3">
                      <ChannelDropdown
                        ...
                        onChange={(ids) => updateChannels(row._key, ids)}
                      />
                    </td>
                    {/* Principal */}
                    <td className="px-4 py-3">
                      <input
                        type="text"
                        value={row.principal}
                        onChange={(e) => updatePrincipal(row._key, e.target.value)}
                        placeholder={pageData!.clientName}
                    ...
                    <td className="px-4 py-3">
                      <input
                        type="text"
                        value={row.brand}
                        onChange={(e) => updateBrand(row._key, e.target.value)}
                        placeholder={pageData!.clientName}
                        className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#3D6273] focus:ring-offset-1"
                      />
                    </td>
                    ...
                    {activeCustomFields.map((f) => (
                      <td key={f.id} className="px-4 py-3">
                        {f.type === "select" ? (
                          <select
                            value={row.customFields[f.id] ?? ""}
                            onChange={(e) => updateCustomField(row._key, f.id, e.target.value)}
              ...
              <div key={row._key} className="border border-gray-200 rounded-xl p-4 bg-white space-y-3">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    ...
                    onClick={() => deleteRow(row._key)}
                    disabled={rows.length === 1}
                    className="text-gray-300 hover:text-red-400 text-xl disabled:opacity-30"
                  >×</button>
                </div>
                {/* Role */}
                <div>
                ...
                <div>
                  ...
                  <ChannelDropdown channels={pageData!.channels} selected={row.channels} onChange={(ids) => updateChannels(row._key, ids)} />
                </div>
                {/* Principal */}
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Principal</label>
                  <input type="text" value={row.principal} onChange={(e) => updatePrincipal(row._key, e.target.value)} placeholder={pageData!.clientName}
                    className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2" />
                </div>
                {/* Brand */}
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Brand</label>
                  <input type="text" value={row.brand} onChange={(e) => updateBrand(row._key, e.target.value)} placeholder={pageData!.clientName}
                    className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2" />
                </div>
                {/* Custom fields */}
                {activeCustomFields.map((f) => (
          ...
          <div className="px-6 py-4 border-t border-gray-100">
            <button
              type="button"
              onClick={() => setRows((prev) => [...prev, makeRow(pageData!.clientName)])}
        ...
        <div className="flex justify-end">
          <button
            ...
            disabled={submitting}
            className="bg-[#3D6273] text-white font-semibold text-sm px-8 py-3 rounded-xl hover:bg-[#2d4e5e] transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {submitting && <span className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full inline-block"></span>}
            {submitting ? "Submitting…" : pageData!.alreadySubmitted ? "Update Submission" : "Submit Personnel Information"}
          </button>
        </div>

        <p className="text-xs text-center text-gray-400 mt-6">
          This form is provided by OuterJoin ARIA. Your information is handled securely.
        </p>
      </div>
    </div>
  );
